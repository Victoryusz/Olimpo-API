...
def enviar_acao(evento: EventoJogador):
    ...
    try:
        ...
        print(
            f"Cronos respondeu com sucesso. ID do Evento: {cronos_resposta.get('id_evento')}"
        )
        return RespostaSucesso(
            mensagem="Ação enviada para Cronos.",
            id_evento=cronos_resposta.get("id_evento"),
        )

    except requests.exceptions.RequestException as e:
        print(f"Erro ao conectar com Cronos: {e}")
        return RespostaSucesso(
            mensagem="Falha ao enviar ação para Cronos.", id_evento="erro"
        )
